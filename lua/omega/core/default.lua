...
local function default_keymaps()
    ...
    map("n", "gr", function() vim.lsp.buf.references() end, { desc = "LSP: List References" })
    ...
    map("n", "<leader>e", function() vim.diagnostic.open_float() end, { desc = "Floating Diagnostic" })
    ...
    map("i", "<C-n>", function() require("blink.cmp").select_next() end, { silent = true })
    map("i", "<C-p>", function() require("blink.cmp").select_prev() end, { silent = true })

    -- 3. Signature Help
    map("i", "<C-s>", function() vim.lsp.buf.signature_help() end, { desc = "LSP: Signature Help" })
end
